...
mobskill_object.onMobWeaponSkill = function(target, mob, skill)
   local dINT = math.floor(mob:getStat(xi.mod.INT) - target:getStat(xi.mod.INT))
    ...
    hppTarget = xi.mobskills.mobMagicalMove(mob, target, skill, hppTarget, xi.magic.ele.DARK, dmgmod, TP_NO_EFFECT)
    hppTarget = xi.mobskills.mobAddBonuses(mob, target, hppTarget.dmg, xi.magic.ele.DARK)
    hppTarget = xi.mobskills.mobFinalAdjustments(hppTarget, mob, skill, target, xi.attackType.MAGICAL, xi.damageType.DARK, xi.mobskills.shadowBehavior.IGNORE_SHADOWS)

    -- Clamp the HPP reduction to a 75% total cap and a 44% total Minimum
