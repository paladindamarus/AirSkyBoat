...
quest.sections =
{
        ...
        [xi.zone.PHOMIUNA_AQUEDUCTS] =
        {
            ['Wooden_Ladder'] =
            {
                onTrigger = function(player, npc)
                    ...
                    npc:getID() == ID.npc.LADDER_KNOCKING then -- Find a better way to identify exactly which ladder it is in case IDs shift
        ...
        [xi.zone.MISAREAUX_COAST] =
        {
            ['qm_mire_incense'] =
            {
                onTrigger = function(player, npc)
                    ...
                    not player:hasKeyItem(xi.ki.MIRE_INCENSE) then
                        player:addKeyItem(xi.ki.MIRE_INCENSE)
                        return quest:messageSpecial(ID.text.KEYITEM_OBTAINED, xi.ki.MIRE_INCENSE)
                    end
                end,
            },
            ...
            ['qm_alsha'] =
            {
                onTrigger = function(player, npc)
                    ...
                    if player:hasKeyItem(xi.ki.MIRE_INCENSE) and
                    quest:getVar(player, 'prog') == 2 then
            ...
            ['Alsha'] =
            {
                onMobDeath = function(mob, player, isKiller, firstCall)
                    if quest:getVar(player, 'prog') == 3 and isKiller ~= mob then
            ...
            onEventFinish =
            {
                ...
                [557] = function (player, csid, option, npc)
                    ...
                    npcUtil.popFromQM(player, npc, ID.mob.ALSHA, { claim = true, hide = 0 }) then
                        return quest:messageSpecial(ID.text.FOUL_STENCH)
                    end
                end,

                [558] = function(player, csid, option, npc)
                    quest:setVar(player, 'prog', 5)
                    player:addKeyItem(xi.ki.BETTER_HUMES_AND_MANNEQUINS)
                    return quest:messageSpecial(ID.text.KEYITEM_OBTAINED, xi.ki.BETTER_HUMES_AND_MANNEQUINS)
                end,
            },
        },
    ...
    -- Quest complete
    {
        ...
        [xi.zone.MHAURA] =
        {
            ...
            {
                onTrigger = function(player, csid, option, npc)
                    return quest:event(321, { [1] = getMannequins(player),
                                              [2] = xi.mannequin.cost.PURCHASE,
                                              [3] = xi.mannequin.cost.TRADE,
                                              [4] = xi.mannequin.cost.POSE,
                                              [5] = player:getGil(),
                ...
                onTrade = function(player, npc, trade)
                    ...
                    if npcUtil.tradeHasExactly(trade, xi.items.HUME_M_MANNEQUIN) or
                    ...
                    npcUtil.tradeHasExactly(trade, xi.items.GALKA_MANNEQUIN) then
                        return quest:progressEvent(319, { [0] = 2,
                                                          [1] = getMannequins(player), -- Player Mannequin List
                                                          [2] = xi.mannequin.cost.PURCHASE,
                                                          [3] = xi.mannequin.cost.TRADE,
            ...
            {
                [321] = function(player, csid, option, npc)
                    player:PrintToPlayer(string.format("Update 321: Option: %s", option))
                    if option == 0 then
                        -- Purchase a mannequin
                        local richEnough = 0
                        if player:getGil() >= xi.mannequin.cost.PURCHASE then richEnough = 1 end
                        player:updateEvent( { [0] = richEnough, -- Not sure if this is the legitimate use, but it works.
                                              [1] = getMannequins(player),
                                              [2] = option,
                                              })
                    elseif option >= 11 and option <= 18 then
                        -- Pose a mannequin
                        local race = option - 10 -- From 1 to 8, for consistency in lua
                        player:updateEvent( { [0] = 1,
                                              [1] = getMannequins(player),
                                              [2] = option,
                                              [3] = getMannequinPose(player, race),
                                              })
                    end
                end,
            },
            ...
            onEventFinish =
            {
                [319] = function(player, csid, option, npc)
                    ...
                    print(option)
                    if option >= 1 and option <= 8 and
                    player:delGil(xi.mannequin.cost.TRADE) then
                        player:tradeComplete()
                        player:messageSpecial(ID.text.ITEM_OBTAINED, xi.items.HUME_M_MANNEQUIN + option - 1)
                ...
                [321] = function(player, csid, option, npc)
                    ...
                    if option >= 1 and option <= 8 then
                        -- Purchase the mannequin.  Option = race (1-8)
                        if player:delGil(xi.mannequin.cost.PURCHASE) then
                            player:messageSpecial(ID.text.ITEM_OBTAINED, xi.items.HUME_M_MANNEQUIN + option - 1)
                            player:addItem(xi.items.HUME_M_MANNEQUIN + option - 1)
                        end
                    elseif option >= 10 then
                        -- Posing a mannequin
                        if player:delGil(xi.mannequin.cost.POSE) then
                            local race = ((option - 11) % 8) + 1 -- 1 to 8 for lua consistency
                            local pose = math.floor(option / 32)
                            setMannequinPose(player, race, pose)
                        end
                    end
                end,
            },
